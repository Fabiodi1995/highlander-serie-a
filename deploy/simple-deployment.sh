#!/bin/bash
# Deployment semplificato senza Git per Highlander
...
if [ ! -f "package.json" ] || [ ! -d "server" ] || [ ! -d "client" ]; then
    echo "Errore: Esegui dalla directory del progetto"
    exit 1
fi

echo "1. Creazione archivio applicazione..."
...
echo "2. Setup server Hetzner..."

# Script server completo
...
# Nginx config
cat > /etc/nginx/sites-available/highlandergame.it << 'NGINXCONF'
...
# Proxy params
cat > /etc/nginx/proxy_params << 'PROXYCONF'
...
echo "3. Deploy applicazione..."

# Upload archivio
scp highlander-app.tar.gz root@$SERVER_IP:/tmp/

# Installazione app
ssh root@$SERVER_IP << 'APPEOF'
...
echo "4. Configurazione SSL e avvio..."

ssh root@$SERVER_IP << 'SSLEOF'
# SSL automatico
